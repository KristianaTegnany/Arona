...
var SplitFile = function SplitFile() {};
/**
...
SplitFile.prototype.splitFile = function (file, parts) {
  var self = this; // Validate parameters.
  ...
  return Promise.promisify(fs.stat)(file).then(function (stat) {
    ...
    var splitSize = Math.floor(totalSize / parts); // If size of the parts is 0 then you have more parts than bytes.
    ...
    var lastSplitSize = splitSize + totalSize % parts; // Capture the partinfo in here:

    var partInfo = []; // Iterate the parts
...
SplitFile.prototype.splitFileBySize = function (file, maxSize) {
  ...
  return Promise.promisify(fs.stat)(file).then(function (stat) {
    ...
    var totalSize = stat.size; // Number of parts (exclusive last part!)

    var parts = Math.ceil(totalSize / maxSize);
    var splitSize = Math.round(maxSize); // If size of the parts is 0 then you have more parts than bytes.
...
SplitFile.prototype.__splitFile = function (file, partInfo) {
  ...
  return Promise.mapSeries(partInfo, function (info) {
    return new Promise(function (resolve, reject) {
      ...
      var maxPaddingCount = String(partInfo.length).length; // initial part number
      // ex. '0', '00', '000', etc.
      ...
      partFiles.push(partName); // Open up writer

      var writer = fs.createWriteStream(partName); // Pipe reader to writer
