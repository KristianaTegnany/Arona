...
function updateBinary() {
  return new Promise((resolve, reject) => {
    ...
    const dest = path.join(__dirname, `ytdl`, bin); // Get the latest version

    request.get(`https://yt-dl.org/downloads/latest/${bin}`).on('error', e => reject(e)) // Handle errors
    .on('end', () => setTimeout(() => {
    ...
    }, 1000)).pipe(fs.createWriteStream(dest, {
      mode: 0o755
    }));
  });
}
