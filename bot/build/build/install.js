...
var DEFAULT_CDN = 'https://github.com/Medium/phantomjs/releases/download/v1.9.19'; // If the process exits without going through exit(), then we did not complete.
...
var phantomPath = null; // If the user manually installed PhantomJS, we want
...
kew.resolve(true).then(tryPhantomjsInLib).then(tryPhantomjsOnPath).then(downloadPhantomjs).then(extractDownload).then(function (extractedPath) {
  return copyIntoPlace(extractedPath, pkgPath);
}).then(function () {
  var location = getTargetPlatform() === 'win32' ? path.join(pkgPath, 'phantomjs.exe') : path.join(pkgPath, 'bin', 'phantomjs');
...
function writeLocationFile(location) {
  ...
  if (/^[a-zA-Z0-9]*$/.test(platform) && /^[a-zA-Z0-9]*$/.test(arch)) {
    contents += 'module.exports.platform = "' + getTargetPlatform() + '"\n' + 'module.exports.arch = "' + getTargetArch() + '"\n';
  }
...
function findSuitableTempDirectory() {
  var now = Date.now();
  var candidateTmpDirs = [process.env.TMPDIR || process.env.TEMP || process.env.npm_config_tmp, '/tmp', path.join(process.cwd(), 'tmp')];

  for (var i = 0; i < candidateTmpDirs.length; i++) {
    var candidatePath = path.join(candidateTmpDirs[i], 'phantomjs');

    try {
      fs.mkdirsSync(candidatePath, '0777'); // Make double sure we have 0777 permissions; some operating systems
  ...
  console.error('Can not find a writable tmp directory, please report issue ' + 'on https://github.com/Obvious/phantomjs/issues/59 with as much ' + 'information as possible.');
  exit(1);
}
...
function getRequestOptions() {
  ...
  var proxyUrl = process.env.npm_config_https_proxy || process.env.npm_config_http_proxy || process.env.npm_config_proxy;
  ...
  if (proxyUrl) {
    ...
    console.log('Using proxy ' + url.format(proxy)); // Enable proxy

    options.proxy = proxyUrl;
  } // Use the user-agent string from the npm config


  options.headers['User-Agent'] = process.env.npm_config_user_agent; // Use certificate authority settings from npm
  ...
  if (!ca && process.env.npm_config_cafile) {
    try {
      ca = fs.readFileSync(process.env.npm_config_cafile, {
        encoding: 'utf8'
      }).split(/\n(?=-----BEGIN CERTIFICATE-----)/g); // Comments at the beginning of the file result in the first
...
function handleRequestError(error) {
  if (error && error.stack && error.stack.indexOf('SELF_SIGNED_CERT_IN_CHAIN') != -1) {
    ...
    console.error('Something unexpected happened, please report this full ' + 'log at https://github.com/Medium/phantomjs');
    exit(1);
  }
}
...
function requestBinary(requestOptions, filePath) {
  ...
  requestProgress(request(requestOptions, function (error, response, body) {
    ...
    try {
      ...
      bar.tick();
    } catch (e) {// It doesn't really matter if the progress bar doesn't update.
    }
...
function extractDownload(filePath) {
  var deferred = kew.defer(); // extract to a unique directory in case multiple processes are
  ...
  fs.mkdirsSync(extractedPath, '0777'); // Make double sure we have 0777 permissions; some operating systems
  // default umask does not allow write by default.
  ...
  if (filePath.substr(-4) === '.zip') {
    ...
    extractZip(path.resolve(filePath), {
      dir: extractedPath
    }, function (err) {
      if (err) {
...
function getLocationInLibModuleIfMatching(libPath) {
  ...
  if (libModule.location && getTargetPlatform() == libModule.platform && getTargetArch() == libModule.arch) {
    try {
...
function tryPhantomjsInLib() {
  ...
  }).fail(function () {// silently swallow any errors
  });
}
/**
...
function tryPhantomjsOnPath() {
  if (getTargetPlatform() != process.platform || getTargetArch() != process.arch) {
    console.log('Building for target platform ' + getTargetPlatform() + '/' + getTargetArch() + '. Skipping PATH search');
    return kew.resolve(false);
  }

  return kew.nfcall(which, 'phantomjs').then(function (result) {
    phantomPath = result;
    console.log('Considering PhantomJS found at', phantomPath); // Horrible hack to avoid problems during global install. We check to see if
    ...
    if (/NPM_INSTALL_MARKER/.test(contents)) {
      ...
      var globalLocation = getLocationInLibModuleIfMatching(path.resolve(fs.realpathSync(phantomPath), '../../lib/location'));
...
function getDownloadSpec() {
  var cdnUrl = process.env.npm_config_phantomjs_cdnurl || process.env.PHANTOMJS_CDNURL || DEFAULT_CDN;
...
function downloadPhantomjs() {
  ...
  if (!downloadSpec) {
    console.error('Unexpected platform or architecture: ' + getTargetPlatform() + '/' + getTargetArch() + '\n' + 'It seems there is no binary available for your platform/architecture\n' + 'Try to install PhantomJS globally');
    exit(1);
  }
...
function verifyChecksum(fileName, checksum) {
  return kew.resolve(hasha.fromFile(fileName, {
    algorithm: 'sha256'
  })).then(function (hash) {
