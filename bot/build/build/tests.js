...
module.exports = {
  ...
  itCreatesTheZipFile: function itCreatesTheZipFile(test) {
  ...
  theZipFileIsValid: function theZipFileIsValid(test) {
    test.expect(1);
    var dstPath = this.tmpZipExtractionDir.path;
    fs.createReadStream(this.tmpZipFile.path).pipe(unzip.Extract({
      path: dstPath
    })).on('close', function () {
  ...
  theZipFileContainsTheRightFiles: function theZipFileContainsTheRightFiles(test) {
    var dstPath = this.tmpZipExtractionDir.path;
